...
class BooksHomeCubit extends Cubit<BooksHomeState> {
  ...
  void fetchBooks(String userId, {isFirstloading = false}) async {
    try {
      ...
      final List<DocumentSnapshot> books = await _firestoreHelper
          .fetchDocuments('users/$userId/books', 'createdAt');
      final List<Book> bookList =
          books.map((book) => Book.fromDocumentSnapshot(book)).toList();
  ...
  Future<void> deleteBook(String userId, Book book) async {
    try {
      ...
      if (book.imageUrl != null) {
        await _storageHelper.deleteFile(book.imageUrl!);
      }
